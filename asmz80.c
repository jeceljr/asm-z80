/**********************************************************************
                   ...
                   Montador de programas para o Z80
                   --------------------------------

                 Inova Tecnologia e Informatica Ltda
                   Jecel Mattos de Assumpcao Junior

               Escrito Originalmente em agosto de 1985
                  Modificado em abril e maio de 1988
...
#define REL_EXPR       0x04
#define LINE_STR       0x05
#define CODE_SEG       0x06
#define DATA_SEG       0x07
#define MARKER         0x1e
...
main ( argc , argv )
    ...
    printf ( "ณ    Assembler Z80 - Edicao de 22 de Junho,1988    ณ\n" ) ;
    printf ( "รฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤด\n" ) ;
    printf ( "ณCopyright Softec Eng. Sistemas Ltda 1985,1986,1987ณ\n" ) ;
    ...
    while ( * cp1 )
        ...
        * cp2 = '\0' ;
        if ( fname [ 0 ] == '/' )
            ...
            if ( usr_file [ 0 ] == '\0' && i == 0 )
                {
                fprintf ( stderr , "ASMZ80 precisa do nome do Arquivo de Entrada\n" ) ;
                exit ( -1 ) ;
                }
            fnmerge ( fname , usr_drive , usr_dir , usr_file , usr_ext ) ;
            if ( i == 0 && ( source = fopen ( fname , "r" ) ) == NULL )
                {
                fprintf ( stderr , "ASMZ80 nao conseguiu abrir Arquivo de Entrada\n" ) ;
                exit ( -1 ) ;
                }
            if ( i == 1 )
                if ( ( output = fopen ( fname , "wb" ) ) == NULL )
                    {
                    fprintf ( stderr , "ASMZ80 nao conseguiu abrir Arquivo de Saida\n" ) ;
                    exit ( -1 ) ;
                    }
                  else
            ...
            if ( i == 2 )
                if ( ( listing = fopen ( fname , "w" ) ) == NULL )
                    {
                    fprintf ( stderr , "ASMZ80 nao conseguiu abrir Arquivo de Listagem\n" ) ;
                    exit ( -1 ) ;
                    }
                  else
                    {
    ...
    sprintf ( fname , "zasm.tmp" ) ;                        /* nome diferente */
...
first_pass ()
    ...
    for ( ; ; )
        ...
        if ( fgets ( line_buf , 255 , source ) == NULL )     /* se acabou o arquivo houve um erro : o programa nao tinha "end" */
            {
            ++ errors ;
            fprintf ( stderr , "\007ASMZ80 : fim inesperado de arquivo\n" ) ;
            return ;
            } ;
        ...
        * -- cp = '\0' ;             /* transforma a linha em um string */
        ...
        if ( match ( cp , ".org.*." , free_text ) )  /* linha que define origem */
            ...
            if ( ( -- instr ) -> num_bytes == 0 )
                ...
                if ( instr -> num_bytes == 2 || instr -> num_bytes == 3 )
                    putbyte ( instr -> byte2 ) ;     /* tem mais de um byte : manda o segundo tambem */
                switch ( matched_with )         /* tinha operando */
                    {
                    case '?' :  fprintf ( first_draft , "%c%c%s" , ESCAPE , REL_EXPR , free_text ) ;
                                putc ( 0 , first_draft ) ;     /* offset relativo : transforme em absoluto */
                                ++ pc ;
                                break ;
                    case '@' :  fprintf ( first_draft , "%c%c%s" , ESCAPE , WORD_EXPR , free_text ) ;
                    ...
                    case '*' :  fprintf ( first_draft , "%c%c%s" , ESCAPE , BYTE_EXPR , free_text ) ;
                                ...
                                ++ pc ;
                                if ( instr -> num_bytes == 2 && ( instr -> byte1 == ( char ) 0xdd || instr -> byte1 == ( char ) 0xfd ) &&
                                     instr -> byte2 == ( char ) 0x36 )      /* tem dois '*' em pattern */
                                    {                     /* estas instrucoes tem um segundo operando de 8 bits */
                                    cp = free_text ;
                                    while ( * cp ++ ) ;            /* acha a segunda expressao */
                                    fprintf ( first_draft , "%c%c%s" , ESCAPE , BYTE_EXPR , cp ) ;
                                    putc ( 0 , first_draft ) ;     /* expressao de 8 bits */
                                    ++ pc ;
                                    } ;
                                break ;
                ...
                if ( instr -> num_bytes == 3 )   /* nas instrucoes de 3 byte, o terceiro vem depois dos operandos */
                    putbyte ( instr -> byte3 ) ;
                } ;
            } ;
        if ( make_listing )
        ...
        if ( c != ESCAPE )
            ...
            switch ( c = getc ( first_draft ) )     /* sequencia de escape */
                ...
                case WORD_EXPR :    c = get_exp () ;                 /* pega a expressao */
                                    ...
                                    putbyte2 ( c >> 8 ) ;            /* byte mais significativo depois */
                                    break ;
                ...
                case DATA_SEG  :    if ( in_code )
                                    ...
                                    in_code  = FALSE ;    /* estamos nos dados */
                                    break ;
                default        :    fprintf ( stderr , "\007ASMZ80 : erro no arquivo intermediario\n" ) ;  /* sequencia de escape desconhecida : nao deve ocorrer */
                                    ++ errors ;
                                    exit ( -1 ) ;
                } ;
    }
